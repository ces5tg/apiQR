...
import { Text, View, StyleSheet, Button, Animated, StatusBar, Alert } from 'react-native';
...
export default function App( { route } ) {
  ...
  const validarInformacion=async () => {
    ...
    const response=await validarHorario( id_aula, idPersona )
    console.log( "aun no sale" )
    ...
    <View style={ styles.container }>
      ...
      { scanned&&(
        <View style={ styles.buttonContainer }>
          ...
          <TouchableOpacity
            style={ styles.scanAgainButton2 }
            onPress={ () => validarInformacion() }
          >
            <Text style={ styles.buttonSpacer }>Validar</Text>
          </TouchableOpacity>
        </View>
      ) }
